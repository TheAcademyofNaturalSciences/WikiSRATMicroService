...
check_folder_structure(){
    ...
    if [[ ! -f $1/main.py ]]; then
        echo "main.py not exist in $1"
        exit 9
    fi
    echo "main.py exist in $1"
	if [[ ! -f $1/constants.py ]]; then
        echo "constants.py not exist in $1"
        exit 9
    fi
    echo "constants.py exist in $1"
	if [[ ! -f $1/DatabaseAdapter.py ]]; then
        echo "DatabaseAdapter.py not exist in $1"
        exit 9
    fi
    echo "DatabaseAdapter.py exist in $1"
	if [[ ! -f $1/StringParser.py ]]; then
        echo "StringParser.py not exist in $1"
        exit 9
    fi
    echo "StringParser.py exist in $1"
